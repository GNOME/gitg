...
public class UiUtils
{
	...
	public static Gtk.Dialog build_dialog(Gtk.Window parent, Gee.HashMap<string, string> dialog_vars, string? stdout_data, string? stderr_data) {
		var dlg = new Gitg.ResultDialog(parent,
		                                dialog_vars.get("dialog-title"),
		                                dialog_vars.get("dialog-label"));
												   ...
												   BuildObjectVars build_object_vars = null)
	{
		...
		string shortcut = get_config_string(conf, action_key_prefix+"shortcut", null);
		...
		add_from_config(dialog_vars, conf, action_key_prefix, "input-history", "false");
		if ("false" == dialog_vars.get("input-history")) {
			dialog_vars.remove("input-history");
		} else {
			var input_history_path = dialog_vars.get("input-history");
			if ("true" == input_history_path) {
				input_history_path = "~/.config/gitg/history/$group.$name";
			}
			var group = get_config_string(conf, action_key_prefix+"group", "");
			input_history_path = input_history_path.replace("$group.", group.down());
			input_history_path = input_history_path.replace("$name", name.down());

			dialog_vars.set("input-history", Gitg.Utils.expand_home_dir(input_history_path));
		}
		...
		item.activate.connect(() => {
			...
			if (show_output && stdout_data != null) {
				...
				var dlg = build_dialog(parent, dialog_vars, stdout_data, null);
				dlg.show();
			}
			...
			if (exit_status != 0 && show_error) {
				...
				var dlg = build_dialog(parent, dialog_vars, stdout_data, stderr_data);
				dlg.show();
			}
		});
	...
	public static string? run_entry_dialog (Gtk.Window? parent,
		...
		var history_path = get(vars, "input-history", null);
		...
		if (history_path != null) {
			entry = new Gitg.EntryHistory (history_path);
		} else {
			entry = new Gtk.Entry ();
		}
